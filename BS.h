...
class BS : public sketch::BaseSketch {
  public:
    BS(int threshold1, int K, int MEM);
    ~BS();
    void clear();
...
// BS
BS::BS(int threshold1, int K, int MEM) {
...
void BS::Insert(const std::string &str) {
...
std::pair<std::string, int> BS::Query(int k) {
    if (k < _ret.size()) {
...
std::string BS::get_name() { return "BS"; }

uint64_t BS::Hash(const std::string &str) {
    return _bobhash->run(str.c_str(), str.length());
}

uint64_t BS::Hash(uint8_t fp) {
    return _bobhash->run((char *)&fp, 1);
}

bool BS::kickout(int kick_num, uint64_t &hash_value,
