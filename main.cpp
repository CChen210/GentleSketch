...
int main(int argc, char** argv)
{
    ...
    int DA_M;
    for (DA_M=1; 96*DA_M*TOP_d+32*DA_M*CMM_d<=MEM*1024*8; DA_M++);
    if (DA_M%2==0) {
        DA_M--;
    }
    ...
    // preparing GentleSketch
    
    func.push_back(new sketch::GentleSketch::GentleSketch(K, MEM));
 // preparing BS
    
    func.push_back(new BS::BS(10, K, MEM));
    
    // preparing heavykeeper
    
    int hk_M;
    for (hk_M=0; 48*hk_M*HK_d+360*K<=MEM*1024*8; hk_M++);
    if (hk_M%2==0) hk_M--;
    if (hk_M<=0) {
    ...
    long long resns;
    char default_dataset[40]="./zipf_1.5.dat";
